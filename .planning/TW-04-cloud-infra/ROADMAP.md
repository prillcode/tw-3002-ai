...
## Phase 2: D1 Schema & Migrations (4-6 hours)
**Goal:** Deploy the game schema to Cloudflare D1 using D1's native migration tooling.

> **Note:** D1 uses `wrangler d1 migrations` — a separate system from the TW-01 SQLite runner. Schema should be **logically equivalent** to local SQLite but may differ in details (e.g., D1 foreign key behavior).
...
Phase 2 (D1 schema) uses D1's native tooling. Phases 3-5 are core cloud infrastructure. Phases 6-9 are distribution and operations.
...
- [ ] 100+ sector galaxy running in cloud
