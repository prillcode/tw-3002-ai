...
- [ ] `GET /pvp/status` — check if PvP is possible in current sector
- [ ] Player combatant object (same interface as NPC combatant)
- [ ] Loot calculation: 50% credits + 10% cargo (random selection)
...
- [ ] Offline attack queue: store attacks for offline players, process on login
...
Phase 1 is core combat. Phases 2-3 add consequences and awareness. Phase 4 is social. Phase 5 is tuning.
